	...
	<div class="padding-right">
		<?php if ( get_the_company_name() ) { ?>
			...
			<div class="company-info" itemscope itemtype="http://data-vocabulary.org/Organization">
				...
				<div class="content">
					<h4><?php the_company_name( '<strong itemprop="name">', '</strong>' ); ?></h4>
					<?php if ( $website = get_the_company_website() ) : ?>
						<span><i class="fa fa-link"></i> <a class="website" href="<?php echo esc_url( $website ); ?>" itemprop="url" target="_blank" rel="nofollow"><?php esc_html_e( 'Website', 'workscout' ); ?></a></span>
					<?php endif; ?>
				...
				<ul>
					...
					<li>
						...
						<div>
							...
							<span>				
								$<?php echo esc_html( $rate_min ) ?> <?php if(!empty($rate_max)) { echo '- '.$rate_max; } ?> <?php esc_html_e('/ hour','workscout') ?>
							</span>
						</div>
					</li>
					<?php } ?>
					...
					<li>
						...
						<div>
							...
							<span>				
								$<?php echo esc_html( $salary_min ) ?> <?php if(!empty($salary_max)) { echo '- '.$salary_max; } ?>
							</span>
						</div>
					</li>
					<?php } ?>
